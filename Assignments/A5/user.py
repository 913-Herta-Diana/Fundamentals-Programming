from calc import *
print(create_complex_number(3,4))
#
...
def display_mountain(complex_numbers):
    sequence=get_longest_sequence_mountain(complex_numbers)
    display_number_list(sequence)
    display_lenght(sequence)

def display_sum(complex_numbers):
    sequence=get_longest_sequence(complex_numbers)
    display_number_list(sequence)
    display_lenght(sequence)
...
def get_longest_sequence(complex_numbers):
    ...
    :return: Subarray with maxim sum
...
def get_longest_sequence_mountain(complex_numbers):
    ...
    :param complex_numbers: list of complex numbers
    :return: longest mountain sequence
    """
    best_sequence = []
    for i in range(len(complex_numbers)):
...
def isMountainArray(arr):
    """
    :param: sequence that needs to be checked
    :return: True or False if the sequence is a mountain
    """
    if (len(arr) < 3):
        return False
    i = 0
...
def SubarrayWithMaxSum(nums):
    ...
    :param nums: sequence we check for maximium sum subarrays
    :return: list of the subarray
    """
...
def display_lenght(lung):
    print("The lenght is:", len(lung))

def show_menu():
    print("1. Read a list of complex numbers in 'a + bi' form from the console") #checked
    print("2. Display the entire list of complex numbers") #checked
    print("3.  Display length and elements of a longest subarray where their real part is in the form of a mountain")
    print("4. Display the length and elements of a maximum subarray sum, considering each number's real part")
    print("5. Exit the application")

def start():
    done = False
    complex_numbers = [create_complex_number(7, 0),
                       create_complex_number(11, 3),
                       create_complex_number(10, 3),
                       create_complex_number(4, 5),
                       create_complex_number(10, 0),
                       create_complex_number(12, 0),
    ...
    while not done:
        ...
        if user_choice == 1:
            ...
            display_mountain(complex_numbers)
        elif user_choice == 4:
            display_sum(complex_numbers)
        elif user_choice == 5:
            done = True
            print("Bye bye!")
        else:
